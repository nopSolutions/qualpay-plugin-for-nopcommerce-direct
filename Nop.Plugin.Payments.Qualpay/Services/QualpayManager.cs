...
namespace Nop.Plugin.Payments.Qualpay.Services
{
    ...
    public class QualpayManager
    {
        ...
        private (bool Valid, WebhookEvent<TData> Data, string ErrorMessage) ValidateWebhook<TData>(HttpContext context)
            where TData : class
        {
        ...
        private (IList<Domain.LineItem> Items, decimal TaxAmount) GetItems(Core.Domain.Customers.Customer customer, int storeId, decimal orderTotal)
        {
            ...
            //get tax amount
            var taxAmount = _orderTotalCalculationService.GetTaxTotal(shoppingCart, false);
        ...
        public (bool Valid, WebhookEvent<Subscription> webhookEvent) GetSubscriptionFromWebhookRequest(HttpContext context)
        {
            var result = ValidateWebhook<Subscription>(context);
